...
async function main() {
  ...
  // Clean slate (respects FK cascades)
  ...
  for (let i = 0; i < clubSeed.length; i++) {
    ...
    clubs.push({ club, ownerId: owner.id });
  }
  ...
  for (let i = 0; i < eventSeed.length; i++) {
    const e = eventSeed[i];
    const { club, ownerId } = clubs[e.club];
    ...
    // Some registrations from the enthusiast on the first few events
    if (i < 3) {
  ...
  // Follows
  await prisma.clubFollow.create({ data: { userId: enthusiast.id, clubId: clubs[0].club.id } });
  await prisma.clubFollow.create({ data: { userId: enthusiast.id, clubId: clubs[2].club.id } });
